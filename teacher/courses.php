    ...
    <div class="main-content">
      ...
      <main class="page-body">
        ...
        <div
          ...
          <input class="form-control" type="search" id="search-courses" placeholder="Rechercher un cours…"
            style="max-width:320px; margin-top:0;">
        </div>
              ...
              <div class="card course-card-item" data-title="<?= htmlspecialchars(strtolower($c['titre'])) ?>">
                <div class="course-card-header" onclick="toggleLessons(<?= $c['id'] ?>)">
                  <div>
                    ...
                    <span class="text-muted" style="font-size:.8rem;">Module :
                      <strong><?= htmlspecialchars($c['module_titre']) ?></strong> · <?= count($c['lessons']) ?>
                      leçons</span>
                  </div>
                  <div class="course-actions" onclick="event.stopPropagation();">
                    <button class="btn btn-primary btn-sm"
                      onclick="openAddLessonModal(<?= $c['id'] ?>, '<?= htmlspecialchars(addslashes($c['titre'])) ?>')">Ajouter
                      Leçon</button>
                ...
                <div class="card-body" id="lessons-list-<?= $c['id'] ?>" style="padding:0; display:none;">
                    ...
                    <div style="padding: 20px; text-align:center;" class="text-muted">Aucune leçon dans ce cours. Cliquez sur
                      "Leçon" pour en ajouter une.</div>
                    <?php else: ?>
                    <div style="display:flex; flex-direction:column;">
                        ...
                        <div class="lesson-item-teacher">
                          <div>
                            ...
                            <span style="font-size:.75rem; margin-left: 8px;"
                              class="badge badge-gray"><?= $l['type'] === 'video' ? 'Vidéo' : 'PDF' ?></span>
                            <div style="font-size:.8rem; margin-top:2px;" class="text-muted">
                              Fichier : <a
                                href="../uploads/<?= $l['type'] === 'pdf' ? 'pdfs' : 'videos' ?>/<?= htmlspecialchars($l['fichier']) ?>"
                                target="_blank"
                                style="color:var(--primary-light); text-decoration:underline;"><?= htmlspecialchars($l['fichier']) ?></a>
                            </div>
                            ...
                            <div style="margin-top:6px; font-size:.85rem;">
                                ...
                                <span class="text-muted" style="font-size:.78rem; margin-left:6px;">(<?= $l['nb_questions'] ?>
                                  question(s))</span>
                                <button class="btn btn-ghost btn-sm" style="padding: 2px 6px; font-size:.7rem; margin-left:8px;"
                                  onclick="openAddQuestionModal(<?= $l['quiz_id'] ?>, '<?= htmlspecialchars(addslashes($l['quiz_titre'])) ?>')">Ajouter
                                  Question</button>
                                <?php else: ?>
                                <button class="btn btn-accent btn-sm" style="padding: 2px 6px; font-size:.7rem;"
                                  onclick="openAddQuizModal(<?= $l['id'] ?>, '<?= htmlspecialchars(addslashes($l['titre'])) ?>')">Ajouter
                                  un Quiz</button>
                                <?php endif; ?>
                            </div>
                          </div>
      ...
      <form method="POST">
        ...
        <div class="modal-body" style="max-height: 450px; overflow-y: auto;">
          <div class="form-group">
            ...
            <textarea class="form-control" name="question" required rows="2"
              placeholder="Saisir l'énoncé de la question..."></textarea>
          </div>
          <div class="form-row">
            <div class="form-group">
