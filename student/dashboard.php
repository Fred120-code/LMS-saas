              ...
              <?php else: ?>
                <div style="display:flex;flex-direction:column;gap:18px;">
                  <?php foreach ($modules as $m): ?>
                    <div
                      style="border-bottom: 1px solid var(--border); padding-bottom: 12px; last-child: border-bottom-none;">
                      <div style="display:flex; justify-content:space-between; margin-bottom: 6px;">
                        <strong><?= htmlspecialchars($m['titre']) ?></strong>
                        <span class="text-muted" style="font-size:.8rem;"><?= $m['lessons_done'] ?> /
                          <?= $m['total_lessons'] ?> leçons</span>
                      </div>
                      <div style="display:flex; align-items:center; gap:12px;">
                        ...
                        <span
                          style="font-size:.8rem; font-weight:600; width: 40px; text-align:right;"><?= round($m['pourcentage']) ?>%</span>
                        <a href="courses.php?module_id=<?= $m['id'] ?>" class="btn btn-primary btn-sm btn-icon"
                          style="padding:4px 8px;">➔</a>
                      </div>
                    </div>
                  <?php endforeach; ?>
                </div>
              ...
              <table>
                ...
                <tbody>
                  <?php if (empty($recentResults)): ?>
                    <tr>
                      <td colspan="4" class="text-center text-muted" style="padding:24px;">Aucun quiz passé pour le
                        moment.</td>
                    </tr>
                  <?php else: ?>
                      ...
                      <tr>
                        <td>
                          ...
                          <span class="text-muted"
                            style="font-size:.75rem;"><?= htmlspecialchars($r['lesson_titre']) ?></span>
                        </td>
